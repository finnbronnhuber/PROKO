...
// ── Arcade-Währung: Spielzeit-Konto ─────────────────────────────────────────
// Wechselkurs: 30 Min gelernte Zeit → 5 Min Arcade-Spielzeit  =  10 Arcade-Sekunden
// pro gelernter Minute. Wird beim Verbuchen gelernter Minuten gutgeschrieben.
export const ARCADE_SEC_PER_LEARN_MIN = 10;
// Durchschnittliche Spieldauer pro Arcade-Spiel (Minuten) — historische Grundlage.
// (Das Würfelspiel/casino rechnet jetzt zeitbasiert ab, race/luck zahlen vorab.)
export const ARCADE_GAME_AVG_MIN = { casino: 1, race: 2, luck: 3 };

// ── Arcade-Bezahlsystem: Spielzeit-Kosten je Spiel ──────────────────────────
// Vorab zu zahlende Spielzeit (Minuten). Punkte-Einsätze (RACE_COST/LUCK_COST)
// bleiben zusätzlich bestehen. Das Würfelspiel (casino) zahlt KEINE Vorabkosten,
// sondern wird zeitbasiert pro angebrochener Minute abgerechnet.
export const RACE_PLAYTIME_MIN     = 2;   // Würfel-Rennen: 2 Min vorab
export const LUCK_PLAYTIME_BASE_MIN = 4;  // Risk & Roll: 4 Min für 2 Spieler …
// … +1 Min je weiterem Spieler (3P=5, 4P=6).
export function luckPlaytimeMin(players){
  const p = Math.max(LUCK_MIN_PLAYERS, Math.min(LUCK_MAX_PLAYERS, Number(players) || LUCK_MIN_PLAYERS));
  return LUCK_PLAYTIME_BASE_MIN + (p - 2);
}

// ── Arcade-Spiel "Risk & Roll" (Push-Your-Luck Würfel, online 2–4 Spieler) ──
export const LUCK_ROUNDS      = 4;                       // genau 4 Runden pro Spiel
export const LUCK_COST        = 10;                      // Einsatz (Punkte) pro Spiel
export const LUCK_MIN_PLAYERS = 2;
export const LUCK_MAX_PLAYERS = 4;
export const LUCK_DICE        = ["d6", "d8", "d10", "d12", "d20"]; // spielbare Würfeltypen
// Auszahlung je Platzierung (Index 0 = 1. Platz). Summe == Pot (Spielerzahl × Einsatz),
// Gewinner macht am meisten; bei 4 Spielern geht auch Platz 2 mit Plus raus, untere verlieren.
//   net = Auszahlung − Einsatz(10).  2P: +10/−10 · 3P: +10/0/−10 · 4P: +14/+2/−6/−10
export const LUCK_PAYOUTS = {
  2: [20, 0],
  3: [20, 10, 0],
  4: [24, 12, 4, 0],
};
