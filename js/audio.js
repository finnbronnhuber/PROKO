...
// ── Würfel-Rennen (Arcade, online 1v1) ─────────────────────────────────────
// Überworfen: die gewürfelte Zahl war höher als der Abstand zum Ziel → kurzer,
// absteigender "Womp" (deutlich unterscheidbar von uiError/diceLose).
export function raceOvershoot(){
  playTone(420, "sawtooth", 0.09, 0.10);
  playTone(300, "sawtooth", 0.11, 0.10, 0.09);
  playTone(190, "sine",     0.20, 0.12, 0.18);
  playNoise(0.06, 0.06, 0.02, 500, 0.7);
}
// Rausgeworfen: Figur muss zurück zum Start → dumpfer Aufprall (tiefer Sub-Schlag
// + kurzer tieffrequenter Noise-Burst, wie ein Stoß auf dem Spielbrett).
export function raceKnockback(){
  playNoise(0.16, 0.30, 0, 170, 0.8);
  playTone(75, "sine",     0.30, 0.30);
  playTone(52, "sine",     0.40, 0.18, 0.05);
  playTone(160, "triangle", 0.08, 0.10, 0.01);
}
