    ...
    import { el, clamp, deepClone, genTemplateId } from "./js/utils.js";
    import {
    ...
    import {
      ...
      FLOW_PROMPT_WINDOW_SEC, FLOW_AUTO_FIRST_SEC, FLOW_AUTO_INTERVAL_SEC, FLOW_BREAK_DEFAULT_MIN,
      DICE_BALANCE,
    ...
    (() => {
      ...
      //   • Flow mode → flowResetIfNewDay() (leaves a running block untouched).
      ...
      //     stale interval progress back to a fresh day-0 timer.
      function handleDayRollover(){
        const t = todayISO();
        if(lastSeenDay === null){ lastSeenDay = t; return; }
        if(t === lastSeenDay) return;
        // Don't interrupt a session that's actively running across the cutoff. Leave
        // lastSeenDay unchanged so the reset is retried (and the stale dots cleared)
        // as soon as the user pauses or finishes the block.
        const classicBusy = !isFlow() && state.running
          && (state.phase === "learning" || state.phase === "break");
        const flowBusy = isFlow() && state.flow && state.flow.running;
        if(classicBusy || flowBusy) return;
        lastSeenDay = t;
        flowResetIfNewDay();
        if(!isFlow()){
