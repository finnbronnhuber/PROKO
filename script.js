    (() => {
      ...
      async function authSubmitHandler(){
        ...
        try{
          if(authMode === "login"){
            ...
            // Send reset email; redirect back to the app (with #access_token hash for recovery)
      ...
      async function performAccountDeletion(){
        ...
        try{
          ...
          // Step 2: Wipe the user_profiles row (RLS: only deletes own row).
          //         The auth.users entry must be removed by the user via Supabase Dashboard,
          //         OR via an Edge Function with service_role. From the client we can:
          //         a) delete our app_state row (immediate)
          //         b) sign out (effective logout)
          //         The auth.users entry will be cleaned up server-side (cascade or admin).
          const userId = currentUser.id;
          const { error: delErr } = await sb.from("user_profiles").delete().eq("id", userId);
          if(delErr) console.warn("[deleteAccount] profile delete:", delErr.message || delErr);
          ...
          // After sign-out, show a confirmation note in the (now-visible) auth modal
          setAuthNote("Account und alle zugehörigen Daten wurden gelöscht.");
          uiSave();
        } catch(err){
      ...
      // Apply a template's settings to state.settings (used when switching templates or onboarding)
