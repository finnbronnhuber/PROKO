    ...
    (() => {
      ...
      function loadDice3D(){
        ...
        if(!dice3dPromise){
          dice3dPromise = import("./js/dice3d.js")
      ...
      // ════════════════════════════════════════════════════════════
      let arcadeCurrent = "menu"; // "menu" | "casino" | "race"
      ...
      function raceEnterGame(gameId, isHost, oppName){
        ...
        raceGameChannel.subscribe(async (status) => {
          if(status === "SUBSCRIBED"){
            try{ await raceGameChannel.track({ name: raceMyName() }); }catch(_){}
          } else if(status === "CHANNEL_ERROR" || status === "TIMED_OUT"){
      ...
      function raceOnGamePresence(){
        if(!race || race.phase !== "starting" || !raceGameChannel) return;
      ...
      function raceApplyRoll(mine, value){
        ...
        if(mine) race.myPos = pos; else race.oppPos = pos;
      ...
      function raceSetupDie(){
        ...
        loadDice3D().then(mod => {
          ...
          if(raceDie3d) return;
          const die = mod.createDie3D(wrap, "d6", getActiveSkin("d6"), { size: 150 });
          if(die) raceDie3d = die;
        });
      }
      ...
      function renderRaceView(){
        ...
        if(phase === "idle"){
          ...
          if(badge){
            badge.textContent = race.rolling ? "🎲 …" : (race.myTurn ? "Du bist dran!" : race.oppName + " ist dran…");
      ...
      function applyLoadedState(){
        ...
        // Online-Rennen überlebt keinen Login/Logout-Wechsel (Channels gehören zur Sitzung)
