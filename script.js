    (() => {
      ...
      if(soundDom.wrap){
        soundDom.wrap.addEventListener("mouseenter", showVolumePopover);
        soundDom.wrap.addEventListener("mouseleave", hideVolumePopoverSoon);
        soundDom.popover && soundDom.popover.addEventListener("mouseenter", showVolumePopover);
        soundDom.popover && soundDom.popover.addEventListener("mouseleave", hideVolumePopoverSoon);
        // Mobile / focus access — click the icon to toggle the popover open/close.
        soundDom.btn && soundDom.btn.addEventListener("click", () => {
      ...
      function switchMode(target){
        ...
        if(isFlow()) flowStopAll();
        else { workerStop(); diceTrackingStop(); stopAlarm(); releaseWakeLock();
               if(tickTimer){ clearTimeout(tickTimer); tickTimer = null; }
               tickAnchor = null; state.running = false; state.alarmActive = false; }
        state.mode = target;
        applyModeBodyClass();
        // Refresh tabs
        const tp = document.getElementById("modeTabPomodoro");
