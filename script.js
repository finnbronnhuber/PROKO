    (() => {
      ...
      // ── Cloud persistence (Supabase upsert, debounced + in-flight de-duped) ──
      ...
      async function _persistNow(){
        ...
        try{
          ...
          const { error } = await sb.from("user_profiles").upsert(payload);
          if(error) console.warn("[persist]", error.message || error);
        }catch(e){
          console.warn("[persist] exception", e);
        }finally{
          _persistInFlight = false;
          if(_persistPending){ _persistPending = false; _persistNow(); }
        }
      }
      function persist(){
        if(!sb || !currentUser) return; // no-op until logged in
        if(_persistDebounce) clearTimeout(_persistDebounce);
        _persistDebounce = setTimeout(_persistNow, 600);
      }
      ...
      function setAuthMode(mode){
        ...
        authDom.tabRegister.classList.toggle("active", !isLogin);
        authDom.title.textContent = isLogin ? "Willkommen zurück" : "Account erstellen";
        authDom.sub.textContent = isLogin
          ? "Melde dich an, um deinen Fortschritt überall zu sehen."
          : "Erstelle ein Konto, damit deine Daten sicher in der Cloud bleiben.";
        authDom.submit.textContent = isLogin ? "Einloggen" : "Account erstellen";
        authDom.password.setAttribute("autocomplete", isLogin ? "current-password" : "new-password");
        authClearMessages();
      }
        ...
        try{
          if(authMode === "login"){
            ...
            if(data && data.session){
              // Auto-login (email confirmation disabled in project)
            } else {
              setAuthNote("Account erstellt. Bitte bestätige deine E-Mail und logge dich dann ein.");
              setAuthMode("login");
            }
          }
        } catch(err){
      ...
      async function logout(){
        ...
        try{
          // Flush any pending persist before signing out
          if(_persistDebounce){ clearTimeout(_persistDebounce); _persistDebounce = null; }
          if(currentUser) await _persistNow();
      ...
      async function handleSignedIn(user){
        currentUser = user;
        if(authDom.logoutBtn) authDom.logoutBtn.disabled = false;
        ...
        unblockApp();
        hideLoading();
        // If no row existed yet, write the (default or carried) state now so the row is created
        persist();
      }
      ...
      // Persist any pending changes before the tab unloads
      window.addEventListener("beforeunload", () => {
        if(currentUser && _persistDebounce){
          // Best-effort synchronous flush via fetch keepalive
          try{
            clearTimeout(_persistDebounce);
            const url = SUPABASE_URL + "/rest/v1/user_profiles";
            const body = JSON.stringify({
              id: currentUser.id,
              app_state: buildStateSnapshot(),
              updated_at: new Date().toISOString(),
            });
            // Use sb.auth getSession() synchronously via cached token
      ...
      window.__lernplan = { saveSettings, resetDefaults, startOrPause, skipPhase, resetAll, state, logout, supabase: sb };
    })();
