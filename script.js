    ...
    import { el, clamp, deepClone, genTemplateId } from "./js/utils.js?v=2026-06-13-luck-1";
    import {
      localDateKey, todayISO, yesterdayISO, logicalNow,
      fmtTime, fmtTimeFlow, fmtHoursFromMinutes
    } from "./js/time.js?v=2026-06-13-luck-1";
    import {
    ...
    } from "./js/constants.js?v=2026-06-13-luck-1";
    import {
      setVolumeProvider,
    ...
    } from "./js/audio.js?v=2026-06-13-luck-1";
    ...
    (() => {
      ...
      function setTab(name, opts){
        ...
        // 3D-Würfel freigeben, wenn ihr Tab verlassen wird (WebGL-Ressourcen!)
        if(name !== "dice") teardownCasinoDie3D();
      ...
      function loadDice3D(){
        ...
        if(!dice3dPromise){
          dice3dPromise = import("./js/dice3d.js?v=2026-06-13-luck-1")
      ...
      function renderArcadeMenu(){
        renderArcadeTime();
        // Ø-Spieldauer als (künftige) Kosten je Spiel — aktuell nur Anzeige, keine Sperre.
        document.querySelectorAll(".arcadeCardCost[data-game-cost]").forEach(el => {
          const min = ARCADE_GAME_AVG_MIN[el.dataset.gameCost];
          el.textContent = Number.isFinite(min) ? ("Ø " + min + " Min") : "";
        });
      }
      ...
      function raceBegin(hostStarts){
        ...
        race.phase = "playing";
        // Einsatz abziehen — jeder Client zieht nur seine eigenen Punkte ab.
      ...
      function renderRaceView(){
        ...
        if(phase === "idle"){
          ...
          if(btn) btn.disabled = !sb || !hasD6 || !enough;
          ...
          if(hint){
            ...
            else hint.textContent = "Dein W6-Skin wird im Spiel angezeigt.";
          }
      ...
      function luckJoinQueue(){
        ...
        luck = { phase: "queue", dieType, sides: DICE_SIDES[dieType], playerCount: players, matchInitiated: false, queueCount: 1 };
        renderLuckView();
      ...
      function luckRoundOrder(round, n){
        // Startsitz rotiert pro Runde (faire Reihenfolge), danach im Kreis.
      ...
      function luckHostBegin(){
        ...
        luck.phase = "playing";
        luck.round = 1; luck.turnOrder = luckRoundOrder(1, luck.playerCount);
        luck.turnSeat = luck.turnOrder[0];
        luck.curSum = 0; luck.curRolls = [];
      ...
      function luckSnapshot(){
        return {
          ...
          turnSeat: luck.turnSeat, turnOrder: luck.turnOrder,
          totals: luck.totals, left: luck.left, tbTally: luck.tbTally,
          curSum: luck.curSum, curRolls: luck.curRolls,
      ...
      function luckApplySnapshot(snap){
        ...
        luck.turnSeat = snap.turnSeat; luck.turnOrder = snap.turnOrder || luck.turnOrder;
        luck.totals = snap.totals; luck.left = snap.left; luck.tbTally = snap.tbTally;
        luck.curSum = snap.curSum; luck.curRolls = snap.curRolls;
        luck.lastRoll = snap.lastRoll; luck.log = snap.log; luck.result = snap.result;
        // Lokale Aktions-Sperre lösen: wenn ich nicht (mehr) dran bin, oder ein frischer
        // Zug für mich beginnt (noch keine Würfe, keine Animation ausstehend).
        if(luck.turnSeat !== luck.mySeat){ luck.locked = false; }
        else if(!luck.lastRoll && (luck.curRolls || []).length === 0){ luck.locked = false; luck.rolling = false; }
      ...
      function luckCanAct(){
        return luck && luck.phase === "playing" && luck.turnSeat === luck.mySeat
          && !luck.left[luck.mySeat] && !luck.rolling && !luck.locked;
      }
      ...
      function luckHostApply(seat, action){
        ...
        if(action === "roll"){
          ...
          const bust = luck.curRolls.includes(v);
          ...
          if(bust){
            ...
            luck.curRolls = luck.curRolls.concat(v);
            luck.curSum += v;
            luck.log = luck.players[seat].name + " würfelt " + v + " → " + luck.curSum + " Punkte.";
            luckHostBroadcast(); luckRenderFromState();
          }
        } else if(action === "safe"){
          luck.totals[seat] += luck.curSum;
          luck.log = luck.players[seat].name + " sichert " + luck.curSum + " Punkte (Gesamt " + luck.totals[seat] + ").";
          luck.curSum = 0; luck.curRolls = [];
          luck.lastRoll = null;
          luckHostBroadcast(); luckRenderFromState();
      ...
      function luckHostAdvance(){
        ...
        const order = luck.turnOrder;
        const idx = order.indexOf(luck.turnSeat);
        let nextIdx = idx + 1;
        // Sitze überspringen, deren Spieler das Spiel verlassen haben.
        while(nextIdx < order.length && luck.left[order[nextIdx]]) nextIdx++;
        if(nextIdx < order.length){
          luck.turnSeat = order[nextIdx];
          luck.curSum = 0; luck.curRolls = []; luck.lastRoll = null;
          luck.log = luck.players[luck.turnSeat].name + " ist dran.";
          luckHostBroadcast(); luckRenderFromState();
          return;
        }
        // Runde vorbei → nächste Runde oder Spielende
        if(luck.round < LUCK_ROUNDS){
          luck.round++;
          luck.turnOrder = luckRoundOrder(luck.round, luck.playerCount);
          let s = 0; while(s < luck.turnOrder.length && luck.left[luck.turnOrder[s]]) s++;
          luck.turnSeat = luck.turnOrder[s] != null ? luck.turnOrder[s] : luck.turnOrder[0];
          luck.curSum = 0; luck.curRolls = []; luck.lastRoll = null;
      ...
      function renderLuckSetup(){
        ...
        if(btn) btn.disabled = !sb || !hasDie || !enough;
        ...
        if(hint){
          ...
          else hint.textContent = "Dein " + DICE_LABELS[luckSetupDie] + "-Skin wird im Spiel angezeigt.";
        }
      }
      ...
      function renderLuckGame(){
        ...
        if(board){
          ...
          luck.players.forEach(p => {
            const row = document.createElement("div");
            row.className = "luckPlayerCard" + (p.seat === luck.turnSeat ? " active" : "") + (p.seat === luck.mySeat ? " me" : "") + (luck.left[p.seat] ? " left" : "");
            const dot = document.createElement("span"); dot.className = "luckSeatDot"; dot.style.background = LUCK_SEAT_COLORS[p.seat % 4];
            const nm = document.createElement("span"); nm.className = "luckPlayerName"; nm.textContent = p.name + (p.seat === luck.mySeat ? " (Du)" : "");
            const tot = document.createElement("span"); tot.className = "luckPlayerTotal"; tot.textContent = String(luck.totals[p.seat]);
            row.appendChild(dot); row.appendChild(nm); row.appendChild(tot);
            if(isTb){ const tb = document.createElement("span"); tb.className = "luckPlayerTb"; tb.textContent = "🎲" + luck.tbTally[p.seat]; row.appendChild(tb); }
            board.appendChild(row);
          });
        }
        if(curSumEl) curSumEl.textContent = String(luck.curSum || 0);
        if(rolledList){
          while(rolledList.firstChild) rolledList.removeChild(rolledList.firstChild);
          (luck.curRolls || []).forEach(v => { const c = document.createElement("span"); c.className = "luckRollChip"; c.textContent = v; rolledList.appendChild(c); });
        }
        const canAct = luckCanAct();
        if(rollBtn) rollBtn.disabled = !canAct;
        if(safeBtn){ safeBtn.disabled = !canAct || (luck.curSum || 0) <= 0; }
