    (() => {
      ...
      // ════════════════════════════════════════════════════════════
      let audioCtx = null;
      let audioUnlocked = false;     // becomes true after a user-gesture has resumed the context
      // iOS / Safari requirement: the AudioContext starts "suspended" and may only
      // be resumed synchronously inside a real user-gesture handler. We attach a
      // one-shot unlocker to multiple gesture events and play a 1-sample silent
      // buffer to fully wake the audio pipeline (this is the canonical workaround).
      function unlockAudioGesture(){
        try{
          ...
          if(typeof src.start === "function") src.start(0); else if(typeof src.noteOn === "function") src.noteOn(0);
          audioUnlocked = (audioCtx.state === "running");
          if(audioUnlocked) detachAudioUnlock();
        }catch(_){ /* ignore — will retry on next gesture */ }
      }
      ...
      // Attach BEFORE any other listeners so the very first tap unlocks audio
      ...
      function getCtx(){
        ...
        if(audioCtx && audioCtx.state === "suspended"){
          // resume() returns a Promise; we don't await it because tones are scheduled
          // ahead via ctx.currentTime + delay and will play once it transitions to "running".
          try{ const p = audioCtx.resume(); if(p && p.then) p.catch(() => {}); }catch(e){}
        }
        return audioCtx;
      }
      // Global volume multiplier. Returns 0 when muted/disabled → playTone/Noise skip work.
      ...
      function isLearningActive(){
        // Locks the dice game / reward dice while ANY learning is in progress —
        // Pomodoro learning phase OR Flow learning phase.
        if(state.mode === "flow" && state.flow && state.flow.phase === "learning") return true;
        return state.phase === "learning";
      }
      // ════════════════════════════════════════════════════════════
      //   INTERVAL TASKS  (per-day, per-session to-do lists)
      // ════════════════════════════════════════════════════════════
      const TASK_MAX = 10;       // per interval session
      const INBOX_MAX = 50;      // unsorted inbox can hold a bit more
      const TASK_REMOVE_DELAY_MS = 3000;
      const INBOX_KEY = "inbox";  // sentinel session-key
      ...
      function buildTaskItem(task, sourceKey){
        ...
        // Tooltip with project name when set
        if(task.projectKey && state.projects && state.projects[task.projectKey]){
      ...
      // Shared color palette: also used for profile avatars (AVATAR_COLOR_OPTIONS).
      // Projects may freely share the same color (two projects with same hue are OK).
      ...
      const dom = {
        ...
        views: { timer: el("timerView"), dice: el("diceView"), stats: el("statsView"), leaderboard: el("leaderboardView"), todo: el("todoView"), settings: el("settingsView") },
      ...
      function renderTimerControls(){
        ...
        //   • Flow learning: enabled only inside the 30-sec window after every 10-min mark
        //                    (opens the pause prompt). Visually "armed" with a soft glow.
        ...
        // Prominent pause-choice button — visible ONLY in the 30-sec pause window
        const pauseBtn = document.getElementById("flowPauseChoiceBtn");
        if(pauseBtn){
          const windowOpen = isFlow() && state.flow.phase === "learning" && state.flow.running && flowPauseWindowActive();
          if(windowOpen){
            // Remaining seconds in the current 30-sec window
            const t = state.flow.elapsedSec | 0;
            const remain = Math.max(0, FLOW_PROMPT_WINDOW_SEC - (t % 600));
            const secEl = document.getElementById("flowPauseChoiceSec");
            if(secEl && secEl.textContent !== String(remain)) secEl.textContent = String(remain);
            if(pauseBtn.style.display !== "flex") pauseBtn.style.display = "flex";
      ...
      function drawFlowTrack(){
        ...
        const includeOpenBlock = state.flow.phase === "learning" && state.flow.elapsedSec >= 30;
        // Build item list: completed sessions (box + break-circle), then optional open block
        const items = [];
        ...
        for(let i=0;i<items.length;i++){
          ...
          totalW += items[i].type === "flowBreak" ? CIRC_S : BOX_S;
        }
      ...
      function handleTrackMove(ev){
        ...
        if(isFlow()){
          ...
          schedulePopHide();
          const flowHit = clickBoxes.find(b => (b.kind === "flowSession" || b.kind === "flowBreak" || b.kind === "flowOpen") && inBox(x, y, b));
          if(flowHit){
      ...
      const TAB_TO_ROUTE = {
        timer: "timer", dice: "wuerfelspiel", stats: "stats",
        leaderboard: "leaderboard", todo: "todo", settings: "settings",
      };
      ...
      document.addEventListener("visibilitychange", () => {
        if(document.visibilityState === "visible"){
          ...
          // If the user kept the tab open over midnight, wipe stale flow display
          if(isFlow()){
            const prev = state.flow.phase;
            flowResetIfNewDay();
            if(state.flow.phase !== prev || state.flow.elapsedSec === 0){
              updateTimerUI(); drawTrack();
            }
          }
        }
      ...
      // Format minutes → "Xh YYm" (preferred when ≥ 60 min)
      ...
      // ════════════════════════════════════════════════════════════
      const FLOW_PROMPT_WINDOW_SEC = 30;     // 30-sec window after every full 10-min mark
      const FLOW_AUTO_FIRST_SEC    = 3600;   // first auto-prompt at 60 min
      const FLOW_AUTO_INTERVAL_SEC = 1800;   // every 30 min after that
      const FLOW_BREAK_DEFAULT_MIN = 10;
      ...
      function fmtTimeFlow(sec){
        // Same MM:SS format as the main timer (no hour roll-over for now)
        return fmtTime(Math.max(0, Math.floor(sec)));
      }
      // True if the user is currently inside a 30-second window starting at any full 10-min mark
      // (after at least 10 min of learning). Used to ENABLE the manual pause-prompt button.
      function flowPauseWindowActive(){
        if(!isFlow() || state.flow.phase !== "learning") return false;
        const t = state.flow.elapsedSec | 0;
        if(t < 600) return false;
        return (t % 600) < FLOW_PROMPT_WINDOW_SEC;
      }
      ...
      function flowTimerTick(){
        ...
        if(flowTickAnchor.phase === "learning"){
          ...
          if(remain <= 0){
            // Break ended → start a fresh learning block (counter reset to 0)
            flowEndBreak();
          }
        }
      }
      ...
      // If the flow timer was last touched on a different local day AND it isn't
      // actively running, reset its displayed counter to 0 (fresh new day).
      // Running blocks that span midnight are left alone — the user clearly wants
      // to keep counting their continuous session.
      function flowResetIfNewDay(){
        if(!state.flow) return;
        const today = todayISO();
        const stamped = state.flow.dayKey || "";
        if(stamped === today) return;
        // Don't disrupt an active learning/break in progress
        if(state.flow.running) { state.flow.dayKey = today; return; }
        // Reset the display: idle timer back at 0
        state.flow.elapsedSec = 0;
        state.flow.breakLeftSec = 0;
        state.flow.breakTotalSec = 0;
      ...
      // User skips a flow break manually (mp3 skip button while in break)
